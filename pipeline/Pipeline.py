...
class Pipeline:
    ...
    def convert_message_content_to_dict(self, response_object: Dict) -> Dict:
        ...
        response_object.choices[0].message.content = json.loads(
    ...
    def forward_pipeline_CSG_SR(self, t2s_object: Dict) -> Dict[str, str]:
        ...
        db_path = (
            bird_sql_path + f"/{self.mode}/{self.mode}_databases/{db_id}/{db_id}.sqlite"
        )
        db_description_path = (
            bird_sql_path
            + f"/{self.mode}/{self.mode}_databases/{db_id}/database_description"
        )
        ...
        database_column_meaning_path = (
            bird_sql_path + f"/{self.mode}/column_meaning.json"
        )
        ...
        try:
            ...
            t2s_object["candidate_sql_generation"] = {
                "sql_generation_reasoning": sql_generation_response_obj.choices[
                    0
                ].message.content["chain_of_thought_reasoning"],
            ...
            # execute SQL
            try:
                possible_respose = func_timeout(
                    30, execute_sql, args=(db_path, possible_sql)
                )
            ...
            logging.error(
                f"Error in reaching content from sql generation response for question_id {q_id}: {e}"
            )
            t2s_object["candidate_sql_generation"] = {
        ...
        try:
            predicted_sql = sql_generation_response_obj.choices[0].message.content[
                "SQL"
            ]
        ...
        t2s_object["total_usage"] = {
            "prompt_tokens": t2s_object["candidate_sql_generation"]["prompt_tokens"]
            + t2s_object["sql_refinement"]["prompt_tokens"],
            "completion_tokens": t2s_object["candidate_sql_generation"][
                "completion_tokens"
            ]
            + t2s_object["sql_refinement"]["completion_tokens"],
            "total_tokens": t2s_object["candidate_sql_generation"]["total_tokens"]
            + t2s_object["sql_refinement"]["total_tokens"],
        }
    ...
    def forward_pipeline_CSG_QE_SR(self, t2s_object: Dict) -> Dict:
        ...
        question = t2s_object["question"]
        if self.type == "bird":
            sql_path = os.getenv("BIRD_DB_PATH")
        elif self.type == "spider":
            sql_path = os.getenv("SPIDER_DB_PATH")
        elif self.type == "science":
            sql_path = os.getenv("SCIENCE_DB_PATH")

        # bird_sql_path = os.getenv("BIRD_DB_PATH")
        db_path = (
            sql_path + f"/{self.mode}/{self.mode}_databases/{db_id}/{db_id}.sqlite"
        )
        ...
        if self.type == "bird":
            ...
            # -----
            database_column_meaning_path = (
                sql_path + f"/{self.mode}/column_meaning.json"
            )
            db_column_meanings = db_column_meaning_prep(
                database_column_meaning_path, db_id
            )
        ...
        possible_conditions_dict_list = collect_possible_conditions(
            db_path=db_path, sql=possible_sql
        )

        possible_conditions = sql_possible_conditions_prep(
            possible_conditions_dict_list=possible_conditions_dict_list
        )
        ...
        try:
            enriched_question = q_enrich_response_obj.choices[0].message.content[
                "enriched_question"
            ]
            enrichment_reasoning = q_enrich_response_obj.choices[0].message.content[
                "chain_of_thought_reasoning"
            ]
            t2s_object["question_enrichment"] = {
                "enrichment_reasoning": q_enrich_response_obj.choices[
                    0
                ].message.content["chain_of_thought_reasoning"],
                "enriched_question": q_enrich_response_obj.choices[0].message.content[
                    "enriched_question"
            ...
            enriched_question = (
                question + enrichment_reasoning + enriched_question
            )  # This is added after experiment-24
        except Exception as e:
            logging.error(
                f"Error in reaching content from question enrichment response for question_id {q_id}: {e}"
            )
            t2s_object["question_enrichment"] = {
        ...
        t2s_object["total_usage"] = {
            "prompt_tokens": t2s_object["candidate_sql_generation"]["prompt_tokens"]
            + t2s_object["question_enrichment"]["prompt_tokens"]
            + t2s_object["sql_refinement"]["prompt_tokens"],
            "completion_tokens": t2s_object["candidate_sql_generation"][
                "completion_tokens"
            ]
            + t2s_object["question_enrichment"]["completion_tokens"]
            + t2s_object["sql_refinement"]["completion_tokens"],
            "total_tokens": t2s_object["candidate_sql_generation"]["total_tokens"]
            + t2s_object["question_enrichment"]["total_tokens"]
            + t2s_object["sql_refinement"]["total_tokens"],
        }
    ...
    def forward_pipeline_SF_CSG_QE_SR(self, t2s_object: Dict) -> Dict:
        ...
        try:
            t2s_object["schema_filtering"] = {
                "filtering_reasoning": schema_filtering_response_obj.choices[
                    0
                ].message.content["chain_of_thought_reasoning"],
                "filtered_schema_dict": schema_filtering_response_obj.choices[
                    0
                ].message.content["tables_and_columns"],
            ...
            logging.error(
                f"Error in reaching content from schema filtering response for question_id {q_id}: {e}"
            )
            t2s_object["schema_filtering"] = f"{e}"
            return t2s_object

        ### STAGE 1.1: FILTERED SCHEMA CORRECTION
        filtered_schema_dict = schema_filtering_response_obj.choices[0].message.content[
            "tables_and_columns"
        ]
        filtered_schema_dict, filtered_schema_problems = filtered_schema_correction(
            db_path=db_path, filtered_schema_dict=filtered_schema_dict
        )
        ...
        schema_statement = generate_schema_from_schema_dict(
            db_path=db_path, schema_dict=filtered_schema_dict
        )
    ...
    def construct_question_enrichment_prompt(
        self,
        db_path: str,
        q_id: int,
        db_id: str,
        ...
        """
        enrichment_template_path = os.path.join(
            os.getcwd(), "prompt_templates/question_enrichment_prompt_template.txt"
        )
        question_enrichment_prompt_template = (
            extract_question_enrichment_prompt_template(enrichment_template_path)
        )
        few_shot_data_path = os.path.join(
            os.getcwd(), "few-shot-data/question_enrichment_few_shot_examples.json"
        )
        ...
        schema = generate_schema_from_schema_dict(
            db_path=db_path, schema_dict=schema_dict
        )
    ...
    def question_enrichment_module(
        self,
        db_path: str,
        q_id: int,
        db_id: str,
    ...
    def construct_candidate_sql_generation_prompt(
        self,
        db_path: str,
        db_id: int,
        question: str,
        evidence: str,
        filtered_schema_dict: Dict,
        db_descriptions: str,
    ) -> str:
        """
        ...
        """
        sql_generation_template_path = os.path.join(
            os.getcwd(), "prompt_templates/candidate_sql_generation_prompt_template.txt"
        )
        ...
        filtered_schema = generate_schema_from_schema_dict(
            db_path=db_path, schema_dict=filtered_schema_dict
        )
    ...
    def construct_sql_refinement_prompt(
        self,
        db_path: str,
        db_id: int,
        question: str,
        ...
        """
        sql_generation_template_path = os.path.join(
            os.getcwd(), "prompt_templates/sql_refinement_prompt_template.txt"
        )
        with open(sql_generation_template_path, "r") as f:
            sql_generation_template = f.read()
    ...
    def construct_filtering_prompt(
        self,
        db_path: str,
        db_id: str,
        question: str,
        ...
        """
        schema_filtering_prompt_template_path = os.path.join(
            os.getcwd(), "prompt_templates/schema_filter_prompt_template.txt"
        )
    ...
    def candidate_sql_generation_module(
        self,
        db_path: str,
        db_id: int,
        question: str,
