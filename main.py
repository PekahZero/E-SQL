...
def main(args):
    ...
    if args.type == "bird":
        ...
        sql_path = os.getenv("BIRD_SQL_PATH")
    elif args.type == "spider":
        sql_path = os.getenv("SPIDER_DB_PATH")
    elif args.type == "science":
        sql_path = os.getenv("SCIENCE_DB_PATH")

    args.dataset_path = sql_path

    # 为每个数据库创建了 db_description.csv 文件，包含所有表的列描述
    if args.type == "bird":
        process_all_dbs(
            sql_path, args.mode
        )  # for all databases, creating db_description.csv files which include column descriptions for all talbes

    # set random seed
    random.seed(args.seed)

    # load dataset
    dataset_json_path = sql_path + f"/{args.mode}/{args.mode}.json"
    ...
    for ind, t2s_object in enumerate(dataset):
        ...
        if pipeline.pipeline_order == "CSG-SR":
            ...
            raise ValueError(
                "Wrong value for pipeline_order argument. It must be either CSG-QE-SR or CSG-SR."
            )
        ...
        print(
            f"Question with {q_id} is processed. Correctness: {compare_results['exec_res']} "
        )
...
def calculate_accuracies(predictions: List[Dict]) -> Tuple[float, List]:
    ...
    stats = {
        ...
        "total_item_count": 0,
        "simple": {"correct_number": 0, "count": 0},
        "moderate": {"correct_number": 0, "count": 0},
        "challenging": {"correct_number": 0, "count": 0},
    }
    ...
    if difficulty_flag:
        ...
        stats["simple"]["ex"] = (
            stats["simple"]["correct_number"] / stats["simple"]["count"] * 100
        )
        stats["moderate"]["ex"] = (
            stats["moderate"]["correct_number"] / stats["moderate"]["count"] * 100
        )
        stats["challenging"]["ex"] = (
            stats["challenging"]["correct_number"] / stats["challenging"]["count"] * 100
        )

        stats["total_item_count"] = (
            stats["simple"]["count"]
            + stats["moderate"]["count"]
            + stats["challenging"]["count"]
        )
        stats["total_correct_count"] = (
            stats["simple"]["correct_number"]
            + stats["moderate"]["correct_number"]
            + stats["challenging"]["correct_number"]
        )
...
def check_correctness(t2s_object_prediction: Dict, args) -> Dict[str, Union[int, str]]:
    ...
    db_path = sql_path + f"/{args.mode}/{args.mode}_databases/{db_id}/{db_id}.sqlite"
    if "predicted_sql" in t2s_object_prediction:
        predicted_sql = t2s_object_prediction["predicted_sql"]
        gt_sql = t2s_object_prediction["SQL"]
        compare_results = compare_sqls(
            db_path=db_path, predicted_sql=predicted_sql, ground_truth_sql=gt_sql
        )
    else:
        compare_results = {
            "exec_res": 0,
            "exec_err": "There is no predicted SQL. There must be and error in this question while extracting information.",
        }
...
def create_result_files(args):
    ...
    # 添加数据集类型
    # args.type = "bird" / "spider" / "science"

    # Ensure the results directory exist otherwise create it
    if not os.path.exists("./results"):
        os.makedirs("./results")

    args.output_directory_path = f"./results/{args.type}/model_outputs_{args.mode}_{args.pipeline_order}_{args.model}"
    ...
    predictions_eval_json_path = (
        args.output_directory_path + f"/predict_{args.mode}.json"
    )
...
if __name__ == "__main__":
    ...
    # Model Arguments
    parser.add_argument(
        "--model", default="gpt-4o-mini-2024-07-18", type=str, help="OpenAI models."
    )
    parser.add_argument(
        "--temperature",
        default=0.0,
        type=float,
        help="Sampling temperature between 0 to 2. It is recommended altering this or top_p but not both.",
    )
    parser.add_argument(
        "--top_p",
        default=1,
        type=float,
        help="Nucleus sampling. It is recommend altering this or temperature but not both",
    )
    parser.add_argument(
        "--max_tokens",
        default=2048,
    ...
    parser.add_argument(
        ...
        default="EFG",
        type=str,
        help="The order of stages in the pipeline. It should be either EFG (enrichment --> filtering --> generation) or FEG (filtering --> enrichment --> generation)",
    )
    ...
    parser.add_argument(
        ...
        default="complex",
        type=str,
        help="Defines the which enrichment is used in few-shot examples.It can be either basic or complex.",
    )
    parser.add_argument(
        "-elsn",
        ...
        default=3,
        type=int,
        help="The few-shot number for each difficulty level for question enrichment stage.",
    )
    parser.add_argument(
        "-efsse",
        "--enrichment_few_shot_schema_existance",
        default=False,
        type=str2bool,
        help="Database Schema usage for each few-shot examples in the question enrichment stage. Default False.",
    )

    # Schema Filtering Arguments
    parser.add_argument(
        "-flsn",
        "--filtering_level_shot_number",
        default=3,
        type=int,
        help="The few-shot number for each difficulty level for schema filtering stage.",
    )
    parser.add_argument(
        "-ffsse",
        "--filtering_few_shot_schema_existance",
        default=False,
        type=str2bool,
        help="Database Schema usage for each few-shot examples in the schema filtering stage. Default False.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        type=str2bool,
        help="Whether Context-Free-Grammer or SQL Template will be used. Default is True.",
    )
    parser.add_argument(
        "-glsn",
        "--generation_level_shot_number",
        default=3,
        type=int,
        help="The few-shot number for each difficulty level for SQL generation stage.",
    )
    parser.add_argument(
        "-gfsse",
        "--generation_few_shot_schema_existance",
        default=False,
        type=str2bool,
        help="Database Schema usage for each few-shot examples in the SQL generation stage. Default False.",
    )
    ...
    parser.add_argument(
        ...
        default=5,
        type=int,
        help="The number of value extracted for a column for database samples.",
    )
    ...
    parser.add_argument(
        ...
        default=6,
        type=int,
        help="The number of database item/column descriptions added to a prompt.",
    )
    # custom seed argument
    parser.add_argument("--seed", default=42, type=int, help="Random seed")
    parser.add_argument("--type", default="bird", type=str, help="Dataset type")
